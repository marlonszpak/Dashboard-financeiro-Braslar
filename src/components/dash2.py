...
def generate_visualizations(df):
    ...
    top_values_country = df_atrasos["razao_cliente"].value_counts().head(15).reset_index(name='count')
    ...
    fig_bar_country = px.bar(top_values_country, x='count', y="razao_cliente", orientation='h',
    ...
    top_values_language = df_atrasos["razao_cliente"].value_counts().head(15).reset_index(name='count')
    ...
    top_values = df_atrasos["razao_cliente"].value_counts().head(15).reset_index(name='count')
    # Calculate percentage
    total_count = top_values['count'].sum()
    top_values['percentage'] = (top_values['count'] / total_count) * 100

    fig_bar_language2 = px.bar(top_values, y='count', x="razao_cliente", orientation='v',
                               color='count', text='percentage',
                               title='Top Languages',
